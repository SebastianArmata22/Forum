...
const CreatePost=({category})=>{
    ...
    const [selectCategory, setSelectCategory]=useState(categoryNames[0].name)
    ...
    return(
        <div className='create-post-container'>
            ...
            <select value={selectCategory.id}>
                {categoryNames.map(category=><option key={category.id} value={category.name} onChange={changeSelectCategory}>{category.name}</option>)}
            </select>
